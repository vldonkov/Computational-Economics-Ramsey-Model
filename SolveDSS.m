...
%            General Equilibrium model with one endogenous state variable x
%            and one exogenous shock z.
%
% Algorithm: The policy function is computed on a matrirx of of n by m
%            points, where n is the number of points in the grid for x and
%            m the number of points in the grid for z.
%
%            The method employed is iteration over the value function. The
%            user can choose whether linear interpolation between the grid
%            points of x should be used. For linear interpolation the
%            global variable VI_IP must be set to 1. The default is 0,
%            i.e., no interpolation is used. In this case, the solution
%            matrix xz is a matrix of intergers (i,j), where the (i,j)
%            element of this matrix points to the index of the x that gives
%            the optimal next-period value of the endogenous state
%            variable.
%
%            The  algorithm makes use of the concavity of the value
%            function and the montonicity of the policy function.
%            Specifically, a binary search algorithm is being used in order
%            to locate the maximum on the right-hand side of the Bellman
%            equation.
%
...
%                       order that represent valid elements of the variable
%                       x
%
%            zvec:      m x 1 column vector that represents the
%                       elements of the Markov chain, which represents the
...
%                       point from xvec and j=1, ..., m is the index of a
%                       point from zvec, xz[i,j]=jstar is the index of xvec
%                       that gives the optimal next-period value of the
%                       endogeneous state variable x.
%
% Remarks:   The one-period utility function rf(z,x,x') must be
%            programmed in a procedure lhs=rf(z,x,x') with name rf and the
%            specified ordering of the variables. lhs returns the
%            one-period utility, if the current state of the system is
%            given by (x,z) and if a the next-period state x' is chosen.
...
VI_beta_disc=0; % stores information uses for linear interpolation
...
% Here beings the code for SolveVIS

function [v1,xz]=SolveVIS(beta_disc,xvec,zvec,pmat,v0)
    
    global VI_eps VI_beta_disc VI_xvec VI_zvec VI_ymat VI_pmat VI_Max;
    global VI_nc VI_zex VI_xex;
    
    % Step 1: Initialize
    eps1=VI_eps*(1-beta_disc);
